	...
	static generateSummary(event) {
		return new Promise((fulfill, reject) => {
			event.getConfirms().then(people => {
				...
				summary += "Übersicht für **" + event.name + "** (`#" + event.id + "`):\n\n"

					+ "Das Ereignis " + (status === "expired" ? "war" : "ist") + " geplant für `"
					+ event.time.format("dddd, MMMM Do YYYY, HH:mm")
					+ "\n";

				switch (status) {
					case "pending":
						summary += "Das ist " + event.time.fromNow() + ".\n\n";
						break;
				...
				summary += "\nTo confirm your attendance, type `-ps confirm " + event.id
					+ "`. To deny your attendance, type `-ps confirm " +
					event.id  + " no`\n";
